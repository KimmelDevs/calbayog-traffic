...
export default function SegmentPanel({ segment, currentHour, onClose }) {
  ...
  const flow = segment.series[currentHour]?.flow ?? segment.baseFlow;
  const traffic = getTrafficLevel(flow);
  const max = Math.max(...segment.series.map((s) => s.flow));
  const peakHour = segment.series.reduce((a, b) => (a.flow > b.flow ? a : b));
  ...
  return (
    ...
    >
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 10, color: "#475569" }}>
            LSTM 24-HOUR FLOW PREDICTION
          </div>
        </div>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 20, fontWeight: 700, color: traffic.color }}>
              {flow.toLocaleString()}
            </div>
      ...
      <div style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
        <div style={{ flex: 1 }}>
          <FlowChart series={segment.series} currentHour={currentHour} />
        </div>
        <div style={{ width: 140, flexShrink: 0 }}>
          ...
          <MiniStat label="DAILY AVG" value={`${Math.round(segment.series.reduce((a, b) => a + b.flow, 0) / 24).toLocaleString()}`} color="#38bdf8" />
        </div>
      </div>
    </div>
  );
...
function FlowChart({ series, currentHour }) {
  ...
  const max = Math.max(...series.map((s) => s.flow)) * 1.1;
  ...
  const areaPath =
    `M ${getX(0)},${getY(series[0].flow)} ` +
    series
      .slice(1)
      .map((s, i) => `L ${getX(i + 1)},${getY(s.flow)}`)
      .join(" ") +
    ` L ${getX(23)},${pad.t + h} L ${getX(0)},${pad.t + h} Z`;

  const cx = getX(currentHour);
  const cy = getY(series[currentHour].flow);
