...
export async function loadModel() {
  ...
  _loadPromise = (async () => {
    ...
    console.log("[LSTM] TensorFlow.js imported, version:", tf.version?.tfjs ?? "unknown");
    _model = await tf.loadLayersModel("/tfjs_model/model.json");
    console.log("[LSTM] ✅ Model loaded successfully");
    console.log("[LSTM] Input shape:", JSON.stringify(_model.inputs[0].shape));
    console.log("[LSTM] Output shape:", JSON.stringify(_model.outputs[0].shape));
    return _model;
  })();
  return _loadPromise;
}
...
export async function predictCongestion(locationName, hour, jsDayOfWeek) {
  ...
  console.log(`[PREDICT] locationEnc=${locationEnc} dayEncoded=${dayEncoded} baseFlow=${baseFlow}`);
