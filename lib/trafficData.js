...
export async function predictCongestion(locationName, hour, jsDayOfWeek, vehicleCount = null) {
  ...
  try {
    ...
    if (histRows && histRows.length > 0) {
      ...
      for (const row of histRows) {
        ...
        grouped[row.hour].push(row.vehicle_count);
      }
  ...
  // Step 2: collect raw vehicle counts for all 8 past hours (oldest → newest).
  // Priority: (a) passed-in vehicleCount for the current hour, (b) DB historical avg, (c) simulation.
  const rawFlows = [];
  for (let i = LOOKBACK - 1; i >= 0; i--) {
    const pastHour = ((hour - i) + 24) % 24;
    let rawCount;
    if (i === 0 && vehicleCount !== null) {
      // Most-recent step: use the just-entered real count
      rawCount = vehicleCount;
    } else if (historyByHour[pastHour] !== undefined) {
      // Use DB historical average for this hour
      rawCount = historyByHour[pastHour];
    } else {
  ...
  const sequence = rawFlows.map((step, idx) => {
    ...
    return buildFeatureStep(locationEnc, scaledFlow, step.pastHour, dayEncoded, lag1, lag2, lag3, lag4, rollingMean);
  });
