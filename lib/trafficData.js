...
function buildFeatureStep(locationEnc, scaledFlow, hour, dayEncoded, isAnomaly = 0) {
  ...
  // FIX: Rolling_Mean_4 (index 6) is stored as raw vehicle count in training
  // data (range 19–143), NOT as a scaled 0–1 value. The old code sent
  // scaledFlow here, which put every input out-of-distribution and caused
  // the model to collapse to MODERATE for all predictions.
  const rawCount = unscale(scaledFlow);

  return [
    locationEnc,   // [0] Location_encoded
    scaledFlow,    // [1] Vehicle_Count_scaled
    scaledFlow,    // [2] Scaled_lag1
    scaledFlow,    // [3] Scaled_lag2
    scaledFlow,    // [4] Scaled_lag3
    scaledFlow,    // [5] Scaled_lag4
    rawCount,      // [6] Rolling_Mean_4  ← raw count, NOT scaled
...
export async function predictCongestion(locationName, hour, jsDayOfWeek, vehicleCount = null) {
  console.log(`[PREDICT] Road: "${locationName}" | Hour: ${hour}:00 | Day: ${["Sun","Mon","Tue","Wed","Thu","Fri","Sat"][jsDayOfWeek]}`);
  ...
  for (let i = LOOKBACK - 1; i >= 0; i--) {
    ...
    const rawCount   = vehicleCount !== null ? vehicleCount : simulateLSTMFlow(baseFlow, pastHour, jsDayOfWeek);
    const scaledFlow = minMaxScale(rawCount);
  ...
  console.log(`[PREDICT] ✅ Result: ${label} (${confidence}%) | LIGHT=${Math.round(probs[0]*100)}% MODERATE=${Math.round(probs[1]*100)}% TRAFFIC=${Math.round(probs[2]*100)}%`);
...
export function simulateLSTMFlow(baseFlow, hour, dayOfWeek) {
  ...
  const weekendFactor = (dayOfWeek === 0 || dayOfWeek === 6) ? -0.25 : 0;
  const noise         = (Math.random() - 0.5) * 0.08;

  const signal = Math.max(0, Math.min(1,
    0.1 + peakMorning * 0.6 + peakEvening * 0.8 + nightDip + weekendFactor + noise
  ));
