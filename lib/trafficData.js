...
export async function predictCongestion(locationName, hour, jsDayOfWeek, vehicleCount = null, isAnomaly = 0) {
  ...
  try {
    const { data: histRows } = await supabase
      .from("traffic_logs")
      .select("hour, vehicle_count, is_anomaly")
    ...
    if (histRows && histRows.length > 0) {
      // Group by hour (decimal) and average; track anomaly if ANY entry for that slot was anomalous
      const grouped = {};
      for (const row of histRows) {
        const key = Math.round(row.hour * 4) / 4; // round to nearest 0.25
        if (!grouped[key]) grouped[key] = { counts: [], anomaly: false };
        grouped[key].counts.push(row.vehicle_count);
        if (row.is_anomaly) grouped[key].anomaly = true;
      }
      for (const [h, { counts, anomaly }] of Object.entries(grouped)) {
        historyByHour[Number(h)] = {
          rawCount: Math.round(counts.reduce((a, b) => a + b, 0) / counts.length),
  ...
  for (let i = LOOKBACK - 1; i >= 0; i--) {
    ...
    if (i === 0 && vehicleCount !== null) {
      ...
      stepAnomaly = isAnomaly; // use the flag passed in for the current slot
    } else if (historyByHour[roundedPastHour] !== undefined) {
      rawCount    = historyByHour[roundedPastHour].rawCount;
      stepAnomaly = historyByHour[roundedPastHour].isAnomaly;
    } else {
      // Fall back to simulation using decimal hour
      rawCount    = simulateLSTMFlow(baseFlow, pastHour, jsDayOfWeek);
      stepAnomaly = 0;
    }
    rawFlows.push({ pastHour, rawCount, stepAnomaly });
  }
  ...
  const sequence = rawFlows.map((step, idx) => {
    ...
    return buildFeatureStep(locationEnc, scaledFlow, step.pastHour, dayEncoded, lag1, lag2, lag3, lag4, rollingMean, step.stepAnomaly);
  });
  ...
  // The LSTM feature weight for is_anomaly may be weak since retraining isn't
  // possible at runtime. Instead, if the current slot is anomalous, we shift
  // the probability mass toward TRAFFIC proportional to anomaly severity.
  // Count how many steps in the sequence were anomalous (0–LOOKBACK).
  const anomalyStepCount = rawFlows.filter(s => s.stepAnomaly).length;
  let finalProbs = [...probs]; // [LIGHT, MODERATE, TRAFFIC]

  if (anomalyStepCount > 0) {
    // Bias strength: 0.15 per anomalous step, capped at 0.6
    const biasMagnitude = Math.min(anomalyStepCount * 0.15, 0.60);
    // Shift mass from LIGHT and MODERATE toward TRAFFIC
    const trafficBoost  = biasMagnitude;
    const lightBleed    = trafficBoost * 0.6; // take more from LIGHT
    const moderateBleed = trafficBoost * 0.4; // take less from MODERATE
    finalProbs[0] = Math.max(0, finalProbs[0] - lightBleed);
    finalProbs[1] = Math.max(0, finalProbs[1] - moderateBleed);
    finalProbs[2] = Math.min(1, finalProbs[2] + trafficBoost);
    // Re-normalize so they sum to 1
    const total = finalProbs.reduce((a, b) => a + b, 0);
    finalProbs = finalProbs.map(p => p / total);
    console.log(`[PREDICT] ⚠️ Anomaly bias applied (${anomalyStepCount} steps) | boost=${(trafficBoost*100).toFixed(0)}%`);
  }

  const finalClassIdx  = finalProbs.indexOf(Math.max(...finalProbs));
  const finalLabel     = CLASS_NAMES[finalClassIdx];
  const finalConfidence = Math.round(finalProbs[finalClassIdx] * 100);

  console.log(`[PREDICT] ✅ Result: ${finalLabel} (${finalConfidence}%) | LIGHT=${Math.round(finalProbs[0]*100)}% MODERATE=${Math.round(finalProbs[1]*100)}% TRAFFIC=${Math.round(finalProbs[2]*100)}% | vehicleCount=${actualVehicleCount} | anomalySteps=${anomalyStepCount}`);

  return {
    label:      finalLabel,
    confidence: finalConfidence,
    probabilities: {
      LIGHT:    Math.round(finalProbs[0] * 100),
      MODERATE: Math.round(finalProbs[1] * 100),
      TRAFFIC:  Math.round(finalProbs[2] * 100),
    },
